      ...
      <script>
        ...
        async function saveFallbackRoute() {
          ...
          const res = await fetch('/api/fallback-models', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, modelsText, allowShort: target.mode === 'existing', origin: 'ui', force: true })
          });
          const payload = await res.json().catch(() => ({}));
          if (!res.ok) {
        ...
        async function autoSaveFallbackRoute() {
          ...
          try {
            const res = await fetch('/api/fallback-models', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ id, modelsText, allowShort: true, origin: 'ui', force: true })
            });
            if (res.ok) {
              await loadFallbackRoutes();
            } else {
              const payload = await res.json().catch(() => ({}));
              setMessage('Chain auto-save failed: ' + (payload?.error || ('HTTP ' + res.status)), 'error');
            }
          } catch (e) {
            setMessage('Chain auto-save failed: ' + (e?.message || e), 'error');
          }
        }
