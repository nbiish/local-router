...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            // 1. Ensure backend daemon is active on 11434
            daemon_clone.ensure_started();

            // 2. Setup native corner system tray icon
            tray::setup_tray(app.handle())?;

            // 3. Show and focus main application window
            if let Some(window) = app.get_webview_window("main") {
                let _ = window.show();
                let _ = window.set_focus();
            }
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                // Closing the window minimizes to corner tray so background routing continues
                let _ = window.hide();
                api.prevent_close();
            }
        })
