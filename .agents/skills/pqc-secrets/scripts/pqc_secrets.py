...
ENVELOPE_VERSION = 1
ENVELOPE_ALG = "ML-KEM-768+ML-DSA-65"
ENVELOPE_SIG_DOMAIN = b"pqc-secrets:v1:envelope:sig"
ENVELOPE_DATA_AAD = b"pqc-secrets:v1:envelope:data"
VAULT_DSA_SEED_AAD = b"pqc-secrets:vault:v1:dsa-seed"
MLDSA65_VK_LEN = 1952
...
def _envelope_derive_kek(shared_secret: bytes) -> bytes:
    """Rust parity (main.rs derive_kek): SHA3-256(shared || "pqc-secrets:v1:kek")."""
    return _sha3_256(shared_secret + KDF_INFO)


def _envelope_signed_payload(
    version: int,
    alg: str,
    ...
    """Rust parity (issue.rs envelope_signed_payload):
    domain || version_le_u32 || alg || 0x00 || fp_hex || 0x00 || kem_ct || nonce || ct
    """
    return (
        ENVELOPE_SIG_DOMAIN
...
def _mldsa65_public_key_from_b64(signer_pubkey_b64: str):
    from cryptography.hazmat.primitives.asymmetric import mldsa

    pk = base64.b64decode(signer_pubkey_b64)
    if len(pk) != MLDSA65_VK_LEN:
        raise ValueError(
            f"signer_pubkey has wrong length: {len(pk)} bytes (expected {MLDSA65_VK_LEN} for ML-DSA-65)"
        )
    return mldsa.MLDSA65PublicKey.from_public_bytes(pk)


def _load_recipient_ek(path: str) -> bytes:
    """Recipient pub file: lowercase hex of the 1184-byte ML-KEM-768 ek."""
    text = Path(path).read_text().strip()
    raw = bytes.fromhex(text)
    if len(raw) != 1184:
        raise ValueError(f"recipient public key has wrong length: {len(raw)} bytes (expected 1184)")
    return raw


def _vault_load_dsa_seed() -> bytes:
    """Unwrap the vault's ML-DSA-65 seed (64 bytes) — mirrors _vault_load_kem_seed
    but targets the dsa_seed blob and its own AAD. Same passphrase/KDF path."""
    from argon2.low_level import Type, hash_secret_raw
    ...
    try:
        ...
        blob = header["dsa_seed"]
        if blob.get("aad").encode() != VAULT_DSA_SEED_AAD:
            raise ValueError("vault dsa_seed AAD mismatch (fail closed)")
        nonce = base64.b64decode(blob["nonce_b64"])
        ciphertext = base64.b64decode(blob["ciphertext_b64"])
        seed = AESGCM(kek).decrypt(nonce, ciphertext, VAULT_DSA_SEED_AAD)
    except SystemExit:
        raise
    except Exception as e:
        print(f"ERROR: vault unwrap failed (wrong passphrase or tampered vault): {e}", file=sys.stderr)
        sys.exit(1)
    if len(seed) != KEM_SEED_LEN:
        print(f"ERROR: vault ML-DSA-65 seed has wrong length {len(seed)} (expected {KEM_SEED_LEN})", file=sys.stderr)
        sys.exit(1)
    return seed


def _load_mldsa65_signing_key():
    """Vault-first ML-DSA-65 signing key. No vault: PQC_MLDSA_SEED_FILE (raw
    64 bytes or 128-char hex). Fail closed with guidance otherwise."""
    from cryptography.hazmat.primitives.asymmetric import mldsa

    if VAULT_PATH.exists():
        seed = _vault_load_dsa_seed()
        return mldsa.MLDSA65PrivateKey.from_seed(seed)
    seed_file = os.environ.get("PQC_MLDSA_SEED_FILE")
    if seed_file:
        data = Path(seed_file).read_bytes().strip()
        seed = bytes.fromhex(data.decode()) if len(data) > KEM_SEED_LEN else data
        if len(seed) != KEM_SEED_LEN:
            print(f"ERROR: ML-DSA-65 seed has wrong length {len(seed)}", file=sys.stderr)
            sys.exit(1)
        return mldsa.MLDSA65PrivateKey.from_seed(seed)
    print(
        "ERROR: no ML-DSA-65 signing identity. Run 'vault init' or set PQC_MLDSA_SEED_FILE.",
        file=sys.stderr,
    )
    sys.exit(1)


def _mldsa65_vk_raw(vk) -> bytes:
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat

    return vk.public_bytes(Encoding.Raw, PublicFormat.Raw)


def cmd_envelope(argv: list[str]) -> None:
    """Signed cross-machine transfer (v1.4.0): envelope export|import.

    Double-envelope parity with the Rust engine: ML-KEM-768 seal for the
    recipient's 1184-byte ek, AES-256-GCM payload (AAD envelope:data),
    ML-DSA-65 signature over the canonical wire fields (domain envelope:sig).
    Import verifies BEFORE any decapsulation — fail closed.
    """
    sub = argv[0] if argv else ""
    if sub == "export":
        cmd_envelope_export(argv[1:])
    elif sub == "import":
        cmd_envelope_import(argv[1:])
    else:
        print("Usage: pqc_secrets.py envelope export --recipient <PUB> [--in FILE] [--out FILE]", file=sys.stderr)
        print("       pqc_secrets.py envelope import [--in FILE] [--out FILE]", file=sys.stderr)
        sys.exit(1)
...
def cmd_envelope_export(args: list[str]) -> None:
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat

    recipient_path = _flag(args, "--recipient")
    if not recipient_path:
        print("envelope export requires --recipient <PUB>", file=sys.stderr)
        sys.exit(1)
    in_path = _flag(args, "--in")
    out_path = _flag(args, "--out")
    recipient_ek = _load_recipient_ek(recipient_path)

    text = Path(in_path).read_text() if in_path else sys.stdin.read()
    entries: dict[str, str] = {}
    for line in text.splitlines():
        ...
        key, _, value = line.partition("=")
        key = key.strip()
        if not key or not _ENV_NAME_RE.match(key):
            print(f"ERROR: bad key name in input: {key!r}", file=sys.stderr)
            sys.exit(1)
        entries[key] = value.strip()
    if not entries:
        print("ERROR: no secrets found in input", file=sys.stderr)
        sys.exit(1)

    payload_bytes = json.dumps({"secrets": entries}, separators=(",", ":")).encode()
    shared_secret, kem_ct = _native_mlkem.MLKEM768PublicKey.from_public_bytes(recipient_ek).encapsulate()
    kek = _envelope_derive_kek(shared_secret)
    nonce = secrets.token_bytes(12)
    ct = AESGCM(kek).encrypt(nonce, payload_bytes, ENVELOPE_DATA_AAD)
    recipient_fp_hex = _sha3_256(recipient_ek).hex()

    signing_key = _load_mldsa65_signing_key()
    signed = _envelope_signed_payload(ENVELOPE_VERSION, ENVELOPE_ALG, recipient_fp_hex, kem_ct, nonce, ct)
    signature = signing_key.sign(signed)
    vk_raw = _mldsa65_vk_raw(signing_key.public_key())

    envelope = {
        "version": ENVELOPE_VERSION,
        "alg": ENVELOPE_ALG,
        "recipient_key_sha3_256": recipient_fp_hex,
        "signer_pubkey": base64.b64encode(vk_raw).decode(),
    ...
    if out_path:
        ...
        p.chmod(0o600)
        print(f"Envelope written to {p}")
    else:
        sys.stdout.write(rendered + "\n")
    print(
        f"Envelope signed ({len(entries)} secret(s)); signer fingerprint "
        f"sha3:{_sha3_256(vk_raw).hex()[:16]} - verify the fingerprint with the recipient out-of-band"
    )


def cmd_envelope_import(args: list[str]) -> None:
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat

    in_path = _flag(args, "--in")
    out_path = _flag(args, "--out")
    raw = Path(in_path).read_text() if in_path else sys.stdin.read()
    env = json.loads(raw)

    if env.get("version") != ENVELOPE_VERSION or env.get("alg") != ENVELOPE_ALG:
        print("ERROR: unsupported envelope version/alg (fail closed)", file=sys.stderr)
        sys.exit(1)
    kem_ct = base64.b64decode(env["kem_ct_b64"])
    nonce = base64.b64decode(env["nonce_b64"])
    ct = base64.b64decode(env["ct_b64"])

    # 1) Verify the ML-DSA-65 signature BEFORE any decapsulation (fail closed).
    try:
        pub = _mldsa65_public_key_from_b64(env["signer_pubkey"])
        signed = _envelope_signed_payload(
            env["version"], env["alg"], env["recipient_key_sha3_256"], kem_ct, nonce, ct
        )
        pub.verify(base64.b64decode(env["sig"]), signed)
    except SystemExit:
        raise
    except Exception as e:
        print(f"envelope signature verification FAILED: {e}", file=sys.stderr)
        sys.exit(1)

    # 2) Recipient binding: sealed for THIS machine's ek?
    sk = _load_private_key()
    my_ek_raw = _native_mlkem.MLKEM768PrivateKey.from_seed_bytes(sk).public_key().public_bytes(Encoding.Raw, PublicFormat.Raw)
    if _sha3_256(my_ek_raw).hex() != env["recipient_key_sha3_256"]:
        print("envelope was sealed for a different recipient (recipient_key_sha3_256 mismatch)", file=sys.stderr)
        sys.exit(1)

    # 3) Decapsulate -> KEK -> payload.
    shared_secret = _kem_decapsulate(sk, kem_ct)
    kek = _envelope_derive_kek(shared_secret)
    payload = AESGCM(kek).decrypt(nonce, ct, ENVELOPE_DATA_AAD)
    data = json.loads(payload)
    entries = data.get("secrets", data)

    lines = "".join(f"export {k}={_shell_quote(v)}\n" for k, v in entries.items())
    if out_path:
        p = Path(out_path)
        p.write_text(lines)
        p.chmod(0o600)
        print(f"Imported {len(entries)} secret(s) to {p} (0600 - delete after use; keeping plaintext is a PQC violation)")
    else:
...
ENGINE_BUILD_DATE = "2026-09-01"
ENGINE_COMMANDS = "keygen gen pack export verify list rename migrate setup version envelope"
...
USAGE_LINE = "Usage: pqc_secrets.py <keygen|gen|pack|export|verify|list|rename|migrate|version|envelope>"
